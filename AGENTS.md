...
flux-instance` (it pushes a Flux Kustomization CR). **Improvements over the
    ...
    bundle (vs the bealv ingress host / no CA), argus opt-in label gating, and
    **drops the flux-mgmt cert-manager-via-Vault-PKI path** (`cert-vault.yaml`) —
    argus child clusters get certs from their own cert-manager self-signed CA
    (`clusters/mgmt/cert-manager-issuer`), not a shared Vault PKI. **Prereqs on
