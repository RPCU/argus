...
- `rules-certmanager.yaml` - `GrafanaAlertRuleGroup` `certmanager-certificates`
  ...
  > Requires cert-manager Prometheus metrics enabled
  > (`infrastructure/cert-manager/values.yaml` `prometheus.enabled: true`).
  > The cert-manager chart creates a ServiceMonitor on port 9402 automatically;
  > kube-prometheus-stack's default allow-all selector picks it up.
...
by default, but they were never scraped because `prometheus.enabled` was not set
in `infrastructure/cert-manager/values.yaml`. Without scraped metrics, no
cert-manager alert could fire — a certificate expiring tomorrow would produce
zero signal.

Fix: added `prometheus.enabled: true` to the cert-manager values. The chart
creates a ServiceMonitor targeting port 9402 (`/metrics`) in the `cert-manager`
namespace. kube-prometheus-stack's default allow-all `serviceMonitorSelector`
picks it up automatically — no extra scrape config needed.
...
Three alert rules added in `infrastructure/grafana/alerting/rules-certmanager.yaml`:

- `CertManagerCertificateExpiringCritical` (critical, 7d threshold)
- `CertManagerCertificateExpiringWarning` (warning, 30d threshold)
- `CertManagerCertificateNotReady` (warning, 15m for Ready=False)

The cert-manager ServiceMonitor carries no `release: kube-prometheus-stack`
label, so it is only scraped by kube-prometheus-stack's allow-by-default
`NotIn` selector (which excludes known yaook-component monitors). This is
correct — the cert-manager metrics should be scraped on every cluster.
