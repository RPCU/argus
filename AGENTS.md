...
- `dashboards/palworld-dashboard.yaml` - `GrafanaDashboard` CR (folder `Gaming`) for the Palworld dedicated server, sourced from grafana.com dashboard [20421](https://grafana.com/grafana/dashboards/20421-palworld/) with all datasource UIDs rewritten from `${DS_PROMETHEUS}` to `Mimir`. Consumes the `palworld_*` metrics produced by the `palworld-exporter` sidecar in the **atlas** repo (`clusters/production/palworld/`) — the exporter's `ServiceMonitor` lives in atlas (labeled `release: kube-prometheus-stack`), gets scraped by the Sveltos-pushed Prometheus on the production cluster, and is `remote_write`n to central Mimir here. No app in argus produces these metrics; the dashboard is here only because Grafana is mgmt-only.
