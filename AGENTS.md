...
- `values.yaml` - Base Helm values, consumed by the HelmRelease via `valuesFrom`
  (the generated `external-dns-values` ConfigMap) instead of an inline `values:`
...
- `helmrelease.yaml` - HelmRelease `external-dns` (chart v1.21.1). Reads values
  via `valuesFrom` (`external-dns-values` ConfigMap) so workload clusters can
...
- `kustomization.yaml` - Kustomization manifest (no global namespace override —
  ...
  `configMapGenerator` produces the `external-dns-values` ConfigMap from
...
> the `external-dns-workload-values` override ConfigMap (subdomain scoping +
...
- `clusterprofiles/external-dns.yaml` - **Per-child-cluster ExternalDNS with
  SUBDOMAIN ISOLATION**. Gives each OPT-IN workload cluster its own ExternalDNS
...
enabled}`; `dependsOn: flux-instance` (it pushes a Flux Kustomization CR +
  ...
  the `rpcu.lan` Designate zone is owned by the ADMIN/yaook project, so the
  per-project (tenant-scoped) `cloud-controller` app-cred can only manage its
  recordsets because it carries the custom `dns_manager` role AND the yaook
  Designate `policy:` OR-s `role:dns_manager` into the recordset CRUD + zone-read
  targets (`infrastructure/yaook/designate.yaml`). The role + per-project
  assignments are in `clusters/mgmt/crossplane/openstack/dns-manager-role.yaml`
  and `dns_manager` is in the app-cred `roles` restriction (ccm-credential
  Composition). Without this, ExternalDNS authenticates but every zone/recordset
  call is denied (a tenant `member` can't touch the admin-owned zone).
...
- `designate.yaml` - DesignateDeployment (DNS). The `policy:` map, besides
  ...
  per-project `cloud-controller` app-cred that carries `dns_manager` — WITHOUT
  granting admin to tenant credentials. Zone create/update/delete are NOT granted
  to `dns_manager` (ExternalDNS manages recordsets only; the zone is pre-created
  by admin). The `dns_manager` role + per-project assignments live in
  `clusters/mgmt/crossplane/openstack/dns-manager-role.yaml`; it is added to the
  per-project app-cred `roles` restriction in the `ccm-credential` Composition
...
**Last Updated**: July 2026 (Added DNS permission plumbing for the Sveltos `external-dns` ClusterProfile: a custom `dns_manager` Keystone role (`clusters/mgmt/crossplane/openstack/dns-manager-role.yaml`) granting recordset CRUD + zone/recordset read (no zone write/delete), per-project `RoleAssignmentV3` for `cloud-controller` user and `yaook-sys-maint` in lab/production (`dns-manager-role.yaml`), added `dns_manager` to the per-project CCM app-cred `roles` restriction (`infrastructure/crossplane-compositions/composition-ccm-credential.yaml`) plus a `role-dns` Resource step in the same Composition, and custom Designate policy overrides (`infrastructure/yaook/designate.yaml` `policy:` block) OR-ing `role:dns_manager` into the recordset CRUD + zone-read targets so a tenant token can manage recordsets in the shared admin-owned `rpcu.lan` zone without admin. PRIOR: Added a Sveltos `external-dns` ClusterProfile add-on (`infrastructure/sveltos/clusterprofiles/external-dns.yaml`, wired into `clusterprofiles/kustomization.yaml`) giving each OPT-IN workload cluster (`sveltos.argus.rpcu.io/external-dns: enabled`) its own ExternalDNS that syncs `Ingress`/`Gateway HTTPRoute` records into the SHARED OpenStack Designate zone (`rpcu.lan`) under its OWN subdomain `<cluster>.rpcu.lan` with **subdomain isolation** — a per-cluster `domainFilters: [<cluster>.rpcu.lan]` plus a per-cluster TXT ownership registry (`registry: txt`, `txtOwnerId: <cluster>`, `txtPrefix: edns-`) so clusters never collide in the flat zone. Delivered as a Flux takeover (no Sveltos helmChart bootstrap, same rationale as `openstack-ccm`/`openstack-cinder-csi`). Three things are pushed: the per-project `openstack-credentials` Secret (project-scoped Application Credential `clouds.yaml`, templated by Sveltos from the mgmt `cloud-controller-app-creds` Secret via the SAME `OpenStackCluster.spec.identityRef.cloudName` + `templateResourceRefs` pattern `openstack-ccm` uses), the templated `external-dns-workload-values` override ConfigMap, and a Flux Kustomization CR. **To avoid a duplicate `workload/` overlay, the pushed Flux Kustomization points at the SAME base `./infrastructure/external-dns` as mgmt and uses `patches` with `$patch: delete` to strip the four mgmt-only resources from `secret-credentials.yaml` (the ESO `SecretStore`/`ExternalSecret` reading `capo-variables` + the `capo-system` Role/RoleBinding)**, keeping the Namespace + ServiceAccount; the override ConfigMap is appended as a SECOND `valuesFrom` (later wins). To support the per-cluster override, refactored the base `infrastructure/external-dns` to read values via `valuesFrom` an `external-dns-values` ConfigMap (new `values.yaml` + `configMapGenerator` in `kustomization.yaml`, `disableNameSuffixHash: true`) instead of an inline `values:` block; the mgmt `clusters/mgmt/external-dns.yaml` wiring is unchanged. The addon-controller RBAC (`rbac.yaml`) already grants the `OpenStackCluster` + `cloud-controller-app-creds` reads (for `openstack-ccm`), so no RBAC change was needed. Prereq: `cloud-controller-app-creds` (mgmt ns) must carry a `<project>-clouds.yaml` entry for the cluster's project. Simulated the Flux `$patch: delete` against the base with kustomize (only Namespace/ServiceAccount/values-ConfigMap/HelmRelease/HelmRepository remain); `kustomize build infrastructure/sveltos`, `infrastructure/external-dns`, and `clusters/mgmt` all succeed. PRIOR: Added a Sveltos `cert-manager` ClusterProfile add-on (`infrastructure/sveltos/clusterprofiles/cert-manager.yaml`, wired into `clusterprofiles/kustomization.yaml`) — the re-implemented, improved-and-secured version of the bealv flux-mgmt `cert-vault.yaml` that the `vault-auth` add-on deliberately dropped. It gives each OPT-IN workload cluster (`sveltos.argus.rpcu.io/cert-manager: enabled`) a `vault-issuer` cert-manager `ClusterIssuer` backed by a SHARED mgmt Vault PKI **intermediate** CA, with **subdomain isolation** enforced by Vault itself: per cluster Sveltos provisions (via Crossplane `provider-vault`, LOCALLY on mgmt through an `EventSource`/`EventTrigger` that fires once the workload cluster's cert-manager Deployment is up) a `SecretBackendRole cm-<cluster>` on the `pki-int` mount with `allowedDomains: ["<cluster>.rpcu.lan"]` + `allowSubdomains/allowBareDomains/allowWildcardCertificates: true` and `allowGlobDomains/allowAnyName/allowLocalhost/allowIpSans: false`, a `Policy pki-cm-<cluster>` granting ONLY `pki-int/issue|sign/cm-<cluster>`, a per-cluster kubernetes-auth `Backend` at `pki-clusters/<cluster>` (distinct from vault-auth's `clusters/<cluster>`), and an `AuthBackendConfig`/`AuthBackendRole cert-manager` (workload cluster API host + `<cluster>-ca` via `templateResourceRefs`, bound to the workload cluster's cert-manager `vault-auth` SA). On the workload cluster it pushes a Flux Kustomization installing `infrastructure/cert-manager`, the `vault-auth` SA (+ token + `system:auth-delegator` + a `serviceaccounts/token` Role/RoleBinding for cert-manager's TokenRequest), the `root-mgmt` CA inlined into the issuer `caBundle`, and the `vault-issuer` ClusterIssuer authenticating via the `pki-clusters/<cluster>` mount over `https://vault.mgmt.rpcu.lan`. More secure than flux-mgmt's single shared PKI role/AppRole (Vault refuses any CN/SAN outside the cluster's subdomain; no static tokens/secret-IDs distributed). To supply the shared intermediate, added `clusters/mgmt/crossplane/vault/pki-int.yaml` (Crossplane `Mount pki-int` type pki, `SecretBackendIntermediateCertRequest` generating the intermediate key/CSR internally, `SecretBackendConfigUrls`) wired into `clusters/mgmt/crossplane/vault/kustomization.yaml`, plus a new `infrastructure/vault/README.md` "Vault PKI intermediate bootstrap" section documenting the one-time manual step of signing the Vault CSR with the cert-manager `root-mgmt` CA (openssl) and `vault write pki-int/intermediate/set-signed` (the set-signed is not modelled as an MR since the signed PEM does not exist until the manual signing runs). Widened the Sveltos addon-controller RBAC (`infrastructure/sveltos/rbac.yaml`) with `pki.vault.upbound.io/secretbackendroles` (the `auth.vault`/`vault.vault`/`kubernetes.vault` MR groups were already granted for vault-auth). Corrected the vault-auth section note to reflect that the cert-manager-via-Vault-PKI path is now re-implemented here rather than dropped. Field-verified against upbound/provider-vault v2 CRDs (`Mount.maxLeaseTtlSeconds`, `SecretBackendConfigCa.pemBundleSecretRef` — hence the manual `set-signed` instead of an MR). `kustomize build infrastructure/sveltos`, `clusters/mgmt/crossplane/vault`, and `clusters/mgmt` all succeed. PRIOR: Added versioned ClusterClasses (`openstack-default-v1`, `openstack-kamaji-v1`) to work around CAPO's admission webhook making `OpenStackCluster.spec` immutable after creation. When the `openstack-default` ClusterClass `infrastructure.templateRef` was repointed from `-v2` to `-v3` (to fix the `409 SecurityGroupRuleExists` on new clusters), the CAPI topology controller detected a diff on the live mgmt `OpenStackCluster` (which still had the 4 `remoteManagedGroups` Cilium rules from `-v2`) and tried to reconcile via SSA dry-run — CAPO's webhook denied the request with `spec: Forbidden: cannot be modified`. Fix: `openstack-default-v1` preserves the `-v2` templateRef for existing clusters (e.g. `mgmt`), while `openstack-default` points at `-v3` for new clusters; same pattern for kamaji (`openstack-kamaji-v1` → `-v4`, `openstack-kamaji` → `-v5`). Updated `clusters/mgmt/clusters/mgmt.yaml` to `classRef.name: openstack-default-v1` and `clusters/mgmt/clusters/production.yaml` to `classRef.name: openstack-kamaji-v1`. `kustomize build infrastructure/cluster-api-templates` succeeds. PRIOR: Fixed `409 SecurityGroupRuleExists` aborting CAPO's managed-security-group reconcile on fresh clusters (wedged new cluster `testb-xsxlh`: `failed to reconcile security groups ... POST .../security-group-rules ... 409 ... SecurityGroupRuleExists`). Root cause: the `OpenStackClusterTemplate` `managedSecurityGroups` set `allowAllInClusterTraffic: true` (which makes CAPO create allow-ALL rules between the managed `controlplane`/`worker` groups) **and** also carried explicit `remoteManagedGroups: [controlplane, worker]` Cilium data-plane rules (VXLAN UDP 8472 / health TCP 4240 / Hubble TCP 4244 / ICMP). Each of those enumerated rules normalizes to a Neutron rule tuple the allow-all already produces, so on a from-scratch cluster CAPO POSTs both and the second collides with a hard 409, aborting the whole SG reconcile — they were mislabelled "redundant but harmless" but are redundant AND harmful. Fix (immutable `-vN` rotation): added `openstack-default-cluster-v3` to `infrastructure/cluster-api-templates/templates/infracluster.yaml` and `openstack-kamaji-cluster-v5` to `templates/infrakamaji.yaml`, each identical to its predecessor (`-v2` / `-v4`) but with the four `remoteManagedGroups` rules REMOVED, keeping `allowAllInClusterTraffic: true` (which alone covers the entire Cilium overlay) plus only the `0.0.0.0/0`-scoped rules that don't collide (SSH ingress, DNS egress, NodePort range, and — Kamaji only — kubelet 10250). Repointed both ClusterClasses' `infrastructure.templateRef` (`clusterclass.yaml` → `-v3`, `clusterclass-kamaji.yaml` → `-v5`); `-v2`/`-v4` retained only until the rotation is confirmed, then deleted (an `OpenStackClusterTemplate` rotation reconciles the SGs onto the live `OpenStackCluster` without rolling machines). Rewrote the "CAPO managed security groups must open the Cilium overlay" note in Section 8 to warn against ever re-adding `remoteManagedGroups` rules alongside `allowAllInClusterTraffic`. `kustomize build infrastructure/cluster-api-templates` succeeds. PRIOR: Arranged the Sveltos workload-cluster ClusterProfile `dependsOn` into a clean linear spine so ordering matches intent: **Cilium must be fully installed first, then Flux, then every other add-on once Flux is active**. Added `dependsOn: [cilium]` to the `flux` ClusterProfile in `infrastructure/sveltos/clusterprofiles/flux.yaml` — Flux's Operator pods (and the controllers it deploys) can't schedule without a CNI, so the Flux install now waits on the `cilium` profile. The rest of the add-ons (`flux-instance`, `capi-management`, `openstack-ccm`, `external-snapshotter`, `openstack-cinder-csi`, `vault-auth`) already `dependsOn: flux-instance` (which itself `dependsOn: flux`), so the full spine is now `cilium → flux → flux-instance → add-ons`. A cluster that opts OUT of the Sveltos-managed Cilium (`sveltos.argus.rpcu.io/cilium: disabled`, e.g. bring-your-own-CNI via the chihiro `cilium` toggle) is simply not matched by the `cilium` profile's `clusterSelector`, so Sveltos treats the dependency as satisfied and Flux still deploys — no wedge. Applied the same `dependsOn: [cilium]` to the **embedded copy** of the `flux` ClusterProfile that `capi-management` pushes onto workload-turned-management clusters (the `flux-clusterprofile.yaml` entry in the `capi-management-sveltos-profiles` ConfigMap), so a new management cluster gets the identical ordering. `kustomize build infrastructure/sveltos` succeeds. PRIOR: Made the Sveltos `capi-management` ClusterProfile (`infrastructure/sveltos/clusterprofiles/capi-management.yaml`) also install **kamaji** on the new management cluster, so a workload-turned-mgmt cluster can provision `openstack-kamaji`-class workload clusters (whose apiservers run as Kamaji-hosted pods). Added a `kamaji` Flux `Kustomization` CR to the pushed `capi-management-flux-kustomizations` ConfigMap (path `./infrastructure/kamaji`, `dependsOn: cluster-api-providers` — where the kamaji `ControlPlaneProvider` CR already lives — and `wait: false`, since kamaji's bundled etcd PVCs request the `csi-cinder-sc-delete` StorageClass provided by the separate `openstack-integration` profile and shouldn't block this apply). Added a new templated `capi-management-kamaji-values` ConfigMap (a new `policyRefs` entry on the ClusterProfile) that emits a `kamaji-workload-values` ConfigMap in `kamaji-system` overriding the base's hardcoded `kamaji-etcd.clusterDomain` (`mgmt.local`) with `<cluster-name>.local`; the kamaji Flux Kustomization patches the `kamaji` HelmRelease `valuesFrom` to include it (the `kamaji-system` Namespace is also emitted so the ConfigMap can land before Flux reconciles the kamaji base). The kamaji control-plane provider CR itself was already pushed via `cluster-api-providers`; this adds the missing standalone Helm/etcd runtime. `kustomize build infrastructure/sveltos` succeeds. PRIOR: Backed all Flux controllers' ephemeral storage with tmpfs (RAM-backed `emptyDir` `medium: Memory`) instead of the node disk, to eliminate heavy disk I/O from Git clones, artifact unpacking, and kustomize/helm build scratch. Added kustomize patches to BOTH `infrastructure/fluxcd/instances/flux.yaml` (mgmt/openstack) AND the mirrored workload-cluster FluxInstance in `infrastructure/sveltos/clusterprofiles/flux.yaml`. Volume names differ per upstream manifest: source-controller has `data`(/data, artifacts — the main I/O source, sizeLimit 1Gi) + `tmp`(/tmp, 256Mi); kustomize-controller and helm-controller each have a single `temp`(/tmp, 512Mi). Each `emptyDir: {}` is replaced with `emptyDir: {medium: Memory, sizeLimit: <N>}`. Because a Memory `emptyDir`'s usage counts against the container's memory cgroup limit, the container memory `limits` were also raised for headroom on top of the tmpfs (source-controller 1Gi→2Gi, kustomize/helm-controller 1Gi→1536Mi) — otherwise a full tmpfs OOM-kills the pod; the per-volume `sizeLimit` also caps tmpfs growth so a runaway artifact can't exhaust node RAM. `kustomize build infrastructure/sveltos` and `infrastructure/fluxcd/instances` both succeed. PRIOR: Made the Sveltos `oidc-rbac` ClusterProfile (`infrastructure/sveltos/clusterprofiles/oidc-rbac.yaml`) generate a `cluster-admin` `ClusterRoleBinding` per group name listed in the workload `Cluster`'s `chihiro.io/groups` annotation (comma-separated; each `trim`med, empty entries skipped, binding named `oidc-group-<name>`), in addition to the static bare `kube-admin` → `cluster-admin` binding. The previously-static `kube-user` → `cluster-admin` binding was replaced by this dynamic per-group generation. The annotation is read with the proven `templateResourceRefs` + `getResource` pattern: registered the workload CAPI `Cluster` (`cluster.x-k8s.io/v1beta2`, identifier `WorkloadCluster`), marked the policy ConfigMap `projectsveltos.io/template: "true"`, and ranged over `splitList "," (index ((getResource "WorkloadCluster").metadata.annotations | default dict) "chihiro.io/groups")`. `.Cluster.metadata.*` only reliably exposes name/namespace/kind, so the annotation must be read off an explicitly-registered resource; registering the Cluster also makes Sveltos re-template + re-push when the annotation is edited on an existing cluster. The addon-controller RBAC (`infrastructure/sveltos/rbac.yaml`) already grants `cluster.x-k8s.io/clusters` `get/list/watch`. `kustomize build infrastructure/sveltos` succeeds. PRIOR: Created the previously-documented-but-missing `kubernetes-rbac` app on the mgmt cluster — the OIDC group → RBAC bindings that grant access to the mgmt cluster ITSELF, applied directly by Flux rather than via Sveltos. Added `clusters/mgmt/apps/kubernetes-rbac/crb.yaml` (bare `kube-admin` Group → `cluster-admin`, bare `kube-user` Group → `view`; bare names match the shared Zitadel `groupsClaim` Action output and the ClusterClass empty `groupsPrefix`), `clusters/mgmt/apps/kubernetes-rbac/kustomization.yaml`, and the Flux Kustomization wrapper `clusters/mgmt/kubernetes-rbac.yaml` (path `./clusters/mgmt/apps/kubernetes-rbac`, no `dependsOn`, `prune: true`), wired into `clusters/mgmt/kustomization.yaml`. Sveltos's `oidc-rbac` ClusterProfile only pushes equivalent bindings to opt-in WORKLOAD clusters; the mgmt cluster keeps its own local bindings here. Harmless before mgmt kube-apiserver OIDC is enabled (the groups never appear in any token). `kustomize build clusters/mgmt/apps/kubernetes-rbac` succeeds. PRIOR: Fixed the per-cluster CAPO version override not taking effect when the `sveltos.argus.rpcu.io/capo-version` annotation was edited on an existing workload `Cluster`. The `capi-management` ClusterProfile's templated `cluster-api-providers` Flux Kustomization read the annotation via `index (.Cluster.metadata.annotations | default dict) ...`, but Sveltos's built-in `.Cluster` template variable only reliably exposes `metadata.name`/`namespace`/`kind` — arbitrary annotations were not resolved (so the patch was never emitted and the live provider version never changed). Switched to the proven `templateResourceRefs` + `getResource` pattern already used in this file: registered the workload CAPI `Cluster` (`cluster.x-k8s.io/v1beta2`, identifier `WorkloadCluster`) and changed the guard to `index ((getResource "WorkloadCluster").metadata.annotations | default dict) "sveltos.argus.rpcu.io/capo-version"`. Registering the Cluster as a templateResourceRef also makes Sveltos re-template + re-push when the annotation is edited on an existing cluster. Added `cluster.x-k8s.io/clusters` `get/list/watch` to the addon-controller RBAC (`infrastructure/sveltos/rbac.yaml`). Re-verified the Go-template renders no patch for absent/`default` and a correct override for a real version (now via a mocked `getResource`); `kustomize build infrastructure/sveltos` + `clusters/mgmt` succeed. PRIOR: Surfaced the per-cluster CAPO provider version override as a chihiro create/edit form field. `clusters/mgmt/apps/chihiro/cm.yaml` now writes a `sveltos.argus.rpcu.io/capo-version` annotation on the generated `Cluster` CR from a new `capoVersion` parameter — an editable `select` (`kube-admin`-only via `visible_groups`) with options `default` (sentinel) and `v0.14.4`, default `default`, path `metadata.annotations.'sveltos.argus.rpcu.io/capo-version'`. A `select` with a non-empty `default` sentinel is used deliberately: chihiro hard-errors on an empty `{{ chihiro.* }}` create-form placeholder (manager.go resolves empty placeholders to an "unresolved template parameters" error), so the annotation must always carry a value — `default` means "no override". The Sveltos `capi-management` guard was updated from `{{- if $capoVersion }}` to `{{- if and $capoVersion (ne $capoVersion "default") }}` so the `default` sentinel (and empty/absent) is treated as no-override → repo-pinned CAPO version; a real version emits the `patches` block overriding the CAPO `InfrastructureProvider` `.spec.version`. Validated the chihiro param shape against chihiro's own `internal/cluster` package (DiscoverParameters/GetEditableField/GetParameterDefaults/setYAMLPath round-trip) and re-verified the Sveltos Go-template renders no patch for absent/`default` and a correct override for a real version; `kustomize build` of `clusters/mgmt/apps/chihiro` and `infrastructure/sveltos` both succeed. PRIOR: Added a per-cluster CAPO provider version override to the Sveltos `capi-management` ClusterProfile (`infrastructure/sveltos/clusterprofiles/capi-management.yaml`). The `capi-management-flux-kustomizations` ConfigMap is now Sveltos-templated (`projectsveltos.io/template: "true"`): the pushed `cluster-api-providers` Flux Kustomization conditionally emits a `patches` block overriding the CAPO `InfrastructureProvider` `.spec.version` when the target CAPI `Cluster` CR carries the annotation `sveltos.argus.rpcu.io/capo-version` (e.g. `v0.14.4`); when the annotation is absent no patch is emitted and the repo-pinned default from `infrastructure/cluster-api-providers/infrastructure-openstack.yaml` applies unchanged. This is a Cluster-CR annotation (arbitrary version string), not an opt-in label — labels gate which add-ons a cluster receives, the version is carried as an annotation. Verified the Go-template renders to valid YAML (no `patches` block when unset, a correctly-indented override when set) and that `kustomize build infrastructure/sveltos` still succeeds. Documented in the `capi-management` clusterprofile section + dependency-chain entry. PRIOR: Widened Sveltos addon-controller RBAC (`infrastructure/sveltos/rbac.yaml`) to include `create`/`update`/`patch`/`delete` on Vault Crossplane MR API groups (`auth.vault.upbound.io/backends`, `vault.vault.upbound.io/policies`, `kubernetes.vault.upbound.io/authbackendconfigs`+`authbackendroles`) — the vault-auth EventTrigger + nested ClusterProfile deploys these LOCALLY on mgmt via `deploymentType: Local`, but the addon-controller's base ClusterRole only had read access to secrets and OpenStackClusters, causing `authbackendconfigs ... is forbidden` errors on the nested ClusterProfile. PRIOR: Adopted a per-cluster Vault KV-v2 mount convention `secrets-<cluster>`: the Sveltos `vault-auth` add-on already provisions `secrets-<cluster>` mounts/policies for workload clusters, and the mgmt cluster's own out-of-band setup was aligned to match — `clusters/mgmt/crossplane/vault/vault-store.yaml` `path` changed from the generic `secrets` to `secrets-mgmt`, and `clusters/mgmt/crossplane/vault/external-secret.yaml`'s `vault-creds` `remoteRef.key` simplified from `secrets/mgmt/crossplane` to `crossplane` (the redundant prefix is now the mount). `infrastructure/vault/README.md`'s one-time bootstrap updated accordingly (`vault secrets enable -path=secrets-mgmt kv-v2`; the `crossplane` policy now scopes `secrets-mgmt/data|metadata/crossplane*`; `vault kv put secrets-mgmt/crossplane`; stale `pki-int`/`pki` policy lines dropped since argus has no Vault-PKI path). REQUIRES an out-of-band Vault change on the live mgmt Vault (create the `secrets-mgmt` mount + migrate the `crossplane` secret + repoint the `crossplane` policy). PRIOR: Added Sveltos `vault-auth` ClusterProfile add-on for OPT-IN workload clusters, adapted/improved from the bealv `flux-mgmt` repo (`gitops/apps/sveltos/clusterprofiles/external-secret.yaml`): new `infrastructure/sveltos/clusterprofiles/vault-auth.yaml` (wired into `clusterprofiles/kustomization.yaml`). It provisions, per opt-in workload cluster (`sveltos.argus.rpcu.io/vault-auth: enabled`), a dedicated Vault Kubernetes auth backend on the SHARED mgmt Vault so the child cluster can use ESO against `https://vault.mgmt.rpcu.lan` WITHOUT static tokens — each cluster authenticates with its own SA tokens. An `EventSource`/`EventTrigger` fires once the workload cluster's ESO Deployment is up; then (A) on mgmt, Sveltos instantiates Crossplane `provider-vault` MRs (`Policy secrets-<cluster>`, kubernetes-auth `Backend` at `clusters/<cluster>`, `AuthBackendConfig` with the child's API host + `<cluster>-ca` bundle via `templateResourceRefs`, `AuthBackendRole external-secrets` bound to the ESO `vault-auth` SA); and (B) on the workload cluster it pushes a Flux Kustomization installing the argus ESO base (`infrastructure/external-secrets`), the `vault-auth` SA + token + `system:auth-delegator` binding, the `root-mgmt` CA pushed as `root-mgmt-ca-bundle` (so ESO trusts the Gateway TLS), and a `vault-backend` `ClusterSecretStore`. `dependsOn: flux-instance`. Improvements over the source: argus `flux-system` GitRepository + ESO base, kgateway-fronted Vault URL with the `root-mgmt` CA bundle (vs bealv ingress/no CA), opt-in label gating, and DROPS the flux-mgmt cert-manager-via-Vault-PKI path (argus child clusters use their own cert-manager self-signed CA, not Vault PKI). Widened the sveltos addon-controller RBAC (`infrastructure/sveltos/rbac.yaml`) to `get/list/watch` secrets so templates can read the per-cluster `<cluster>-ca` and `root-mgmt` CA. Reuses the existing mgmt Crossplane `provider-vault` + `default` ProviderConfig (`infrastructure/crossplane-vault`, `clusters/mgmt/crossplane/vault`). PRIOR: Added HashiCorp Vault to the mgmt cluster, adapted from the bealv `flux-mgmt` repo (`gitops/apps/vault`): new `infrastructure/vault` base (namespace, `hashicorp` HelmRepository, Vault Helm chart v0.30.0 HelmRelease, README) + new Flux Kustomization `clusters/mgmt/vault.yaml` (dependsOn kgateway + openstack-cinder-csi) wired into `clusters/mgmt/kustomization.yaml`. Configured for **HA with integrated Raft storage** (3-node quorum, no external Consul): `server.standalone.enabled: false`, `server.ha.enabled: true` + `replicas: 3` + `raft.enabled`/`setNodeId: true` (`storage "raft"` + `service_registration "kubernetes"`); readinessProbe ENABLED (route only to unsealed pods), livenessProbe DISABLED (don't kill sealed pods). The 3 replicas require 3 distinct schedulable nodes (chart `required` podAntiAffinity). Adaptations from the source: the chart's bundled Ingress (`vault.bealv-mgmt.lan`, cert-manager issuer `bealv-mgmt`) is disabled (`server.ingress.enabled: false`) in favour of a Gateway API `HTTPRoute` at `vault.mgmt.rpcu.lan` on the shared kgateway `https` Gateway (TLS terminated with the `rpcu-lan-wildcard-tls` cert / root-mgmt CA — no per-app issuer); each replica's `dataStorage` PVC explicitly requests the `cinder-delete` StorageClass since mgmt has no default StorageClass; the `ui` service `externalPort` set to 8200. Vault starts sealed and must be unsealed manually (init/unseal vault-0, then raft join + unseal vault-1/vault-2). PRIOR: Added Sveltos `capi-management` ClusterProfile for bootstrapping new management clusters: deploys the full CAPI/CAPO stack (cert-manager, external-secrets, cluster-api-operator, ORC, CAPI providers, capo-identity, ClusterClass templates) as Flux Kustomization CRs via opt-in label `sveltos.argus.rpcu.io/capi-management: enabled`, transfers the `capo-variables` secret from the mgmt cluster via `templateResourceRefs`, adds `addon-controller-capo-variables-reader` Role/RoleBinding in `capo-system` for secret read access. PRIOR: Collapsed the Sveltos `openstack-ccm`, `openstack-cinder-csi`, and `external-snapshotter` ClusterProfiles onto a SINGLE shared opt-in label `sveltos.argus.rpcu.io/openstack-integration: enabled` (previously `openstack-ccm` used `openstack-ccm: enabled` and the other two used `openstack-cinder-csi: enabled`). One label now opts a workload cluster into the whole OpenStack integration stack (CCM + Cinder CSI + snapshotter). Cinder CSI cannot function without the CCM (it mounts the CCM-owned `cloud-controller-clouds` Secret and needs the CCM to remove the cloud-provider node taint so its pods schedule) and `dependsOn: openstack-ccm`; the snapshotter is a Cinder CSI prerequisite. Gating all three on one label prevents a cluster opting into Cinder without the CCM and wedging on the unsatisfiable dependency. PRIOR: Added `dependsOn: flux-instance` to the Sveltos `external-snapshotter`, `openstack-ccm`, and `openstack-cinder-csi` ClusterProfiles (`infrastructure/sveltos/clusterprofiles/{external-snapshotter,openstack-ccm,openstack-cinder-csi}.yaml`): all three push Flux `Kustomization` CRs (and the CCM/Cinder-CSI ones also `HelmRelease`s) that reference the `flux-system` `GitRepository` and are reconciled by the kustomize/helm controllers — none of which exist until the `flux`/`flux-instance` ClusterProfiles have installed the Flux Operator + FluxInstance (controllers, CRDs, GitRepository source). `openstack-ccm` had NO `dependsOn` before (the direct gap); `openstack-cinder-csi` was only transitively gated via its `openstack-ccm`/`external-snapshotter` deps — `flux-instance` is now listed explicitly on it too for robustness. Depending on `flux-instance` (which itself dependsOn `flux`) makes Sveltos wait for Flux to be provisioned before pushing these Kustomizations. PRIOR: Disabled Cilium's L2-announcement LoadBalancer on OPT-IN workload clusters: the Sveltos `cilium-values` ClusterProfile's pushed Flux `cilium` Kustomization CR (`infrastructure/sveltos/clusterprofiles/cilium-values.yaml`) now patches the HelmRelease with `l2announcements.enabled: false` and `$patch: delete`s the base `CiliumLoadBalancerIPPool`/`CiliumL2AnnouncementPolicy` (mirroring `clusters/mgmt/cilium.yaml`). Fixes `type: LoadBalancer` Services on workload clusters (e.g. lab) getting a Cilium `10.0.0.240-253` IP instead of an OpenStack floating IP — Cilium's L2 announcer was racing the OpenStack CCM (Octavia owns LoadBalancer on workload clusters via the opt-in `openstack-ccm` ClusterProfile). PRIOR: Added Renovate dependency automation: new `renovate.json5` at repo root for the Mend Renovate GitHub App (no auto-merge — manual review for every PR; batched Monday early morning `Europe/Paris`; Dependency Dashboard enabled). Built-in `flux` + `helm-values` managers cover HelmReleases/HelmRepositories (HTTP + OCI) and values-image repo/tag pairs. Eight `customManagers` (regex) cover the rest: GitHub release-download + `raw.githubusercontent` URL-pinned kustomize bases (orc/gateway-api/dragonfly), `?ref=vX` bases (external-snapshotter x2), Crossplane provider/function packages, CAPI provider `version:` fields (via inline `# renovate:` markers added to `infrastructure/cluster-api-providers/*.yaml` incl. `clusterctl-providers.yaml`), Sveltos inline `helmCharts` chartVersion (markers in `infrastructure/sveltos/clusterprofiles/{cilium,flux}.yaml`), kamaji values `tag:` (marker in `infrastructure/kamaji/values.yaml`), plain raw-manifest `image:` tags (rook ceph/toolbox, chihiro), and the npins GitRelease pin in `npins/sources.json`. packageRules group yaook operators, openstack cloud-provider (CCM + Cinder CSI + images), cluster-api providers, cilium and flux-operator (HelmRelease + Sveltos inline kept in sync); kamaji chart (`0.0.0+latest`) is disabled for the flux manager; major updates are un-batched + labelled. Config validated with `renovate-config-validator`. Documented in new Section 5 "Dependency Updates (Renovate)" + root-level file entry. PRIOR: Kamaji workload clusters: opened kubelet port 10250 from `0.0.0.0/0` so the Kamaji hosted control plane (apiserver pods in the mgmt cluster) can proxy `kubectl logs`/`exec`/`attach`/`port-forward`/`top` to the workload nodes' kubelets — CAPO's managed SG only opened 10250 node-to-node, dropping the off-cluster Kamaji CP source IP. Added NEW `openstack-kamaji-cluster-v3` `OpenStackClusterTemplate` in `infrastructure/cluster-api-templates/templates/infrakamaji.yaml` (= `-v2` + the 10250 ingress rule) and repointed the `openstack-kamaji` ClusterClass `infrastructure.templateRef` to `-v3` (immutable-template `-vN` rotation; delete `-v2` once confirmed); documented in the new "Kamaji control plane must open kubelet port 10250" section. PRIOR: updated Sveltos ClusterProfiles for workload clusters: `flux.yaml` changed to `syncMode: OneTime` and FluxInstance now mirrors `infrastructure/fluxcd/instances/flux.yaml` with `cluster.domain` patched to `{{ .Cluster.metadata.name }}.local` and no `sync` block (cilium Kustomization CR handles Cilium reconciliation separately); flux uses `excludeSelector: type: mgmt` to avoid the mgmt cluster; cilium opt-in via label `sveltos.argus.rpcu.io/cilium: enabled`; documented in sveltos section. PRIOR: added Sveltos ClusterProfiles for Cilium bootstrap and Flux operator on workload clusters: `cilium.yaml` (`syncMode: OneTime`) deploys Cilium v1.18.6 via `helmCharts` to bootstrap networking so pods can schedule; PRIOR: added a basic Sveltos install for the mgmt cluster: new `infrastructure/sveltos` base (one-concern-per-file: namespace, helmrepo, core HelmRelease v1.10.0, dashboard HelmRelease v1.10.1 wired for OIDC/PKCE against the shared Zitadel `kubernetes` client, an `oidc-rbac` ClusterProfile that Sveltos pushes to opt-in child clusters (label `sveltos.argus.rpcu.io/oidc-rbac: enabled`) binding both `kube-admin` and `kube-user` Zitadel OIDC groups to `cluster-admin`, and a kgateway `HTTPRoute` at `sveltos.mgmt.rpcu.lan`); new Flux Kustomization `clusters/mgmt/sveltos.yaml` (dependsOn kgateway, `wait: false` for the placeholder dashboard OIDC clientId) wired into `clusters/mgmt/kustomization.yaml`; documented the component, mgmt key-file, and dependency-chain entries. PRIOR: added kube-apiserver OIDC support to the `openstack-default` ClusterClass: new `oidc` object variable + `enabledIf` patch injecting `--oidc-*` apiserver flags onto the control-plane template; added the shared Zitadel `kubernetes` OIDC app (public/native PKCE, no client secret) in `clusters/openstack/crossplane/zitadel/oidc-apps.yaml`; wired the disabled-by-default `oidc` block into `clusters/mgmt/clusters/mgmt.yaml`; documented in cluster-api-templates README. PRIOR: restructured Crossplane layout: split shared bases (infrastructure/crossplane\*) from per-cluster overlays (clusters/<cluster>/crossplane/); moved openstack MRs/zitadel platform/OIDC apps into clusters/openstack/crossplane/ overlay with prune:false safety; added mgmt Crossplane + chihiro OIDC app (crossplane/zitadel/) with ESO key remapping to match chihiro expectations; documented mgmt Crossplane/chihiro OIDC section, updated deployment dependency chains for both clusters; removed stale infrastructure/crossplane-resources/ directory)
