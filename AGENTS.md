...
- `templates/infracluster.yaml` - OpenStackClusterTemplate `openstack-default-cluster-v1` (kept for live clusters; `identityRef` hardcoded to `mgmt-cloud-config`)
- `templates/infracluster-v2.yaml` - OpenStackClusterTemplate `openstack-default-cluster-v2` (current version; `identityRef` NOT hardcoded — injected by variable patch, enabling per-cluster project targeting). `managedSecurityGroups` sets `allowAllInClusterTraffic: true` plus explicit Cilium data-plane rules (VXLAN UDP 8472, health TCP 4240, Hubble TCP 4244, ICMP via `remoteManagedGroups: [controlplane, worker]`) — required because CAPO's default managed SGs only open API/etcd/kubelet/node-port and would otherwise drop Cilium's cross-node overlay (see note in Cluster Safety).
